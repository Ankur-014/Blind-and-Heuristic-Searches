...
while len(fringe) > 0:
    u, g = min(fringe, key = lambda n: n[1] + hsld[n[0]])
    del fringe[fringe.index((u, g))]
    adjDict = graph[u]
    if u == goal:
        break
    for v in adjDict.keys():
        if v not in parent.keys():
            parent[v] = u, g + adjDict[v]
        elif g + adjDict[v] < parent[v][1]:
            parent[v] = u, g + adjDict[v]
            
        fringe.append((v, g + adjDict[v]))
    
dist, curr, path = 0, goal, []
path.append(curr)
while parent[curr][0] != None:
    path.append(parent[curr][0])
    dist = dist + graph[curr][parent[curr][0]]
    curr = parent[curr][0]
